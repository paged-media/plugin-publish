 ...
 * @returns {Uint8Array}
 */
export function pdf_ir_to_paged_wasm(ir_json) {
    const ptr0 = passStringToWasm0(ir_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.pdf_ir_to_paged_wasm(ptr0, len0);
    if (ret[3]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('pdf_import_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
